...
const DEMO_WALLET_NAME = "Stellar Rozo Demo Wallet";
...
export class PasskeyService {
  ...
  constructor() {
    ...
    if (typeof window === "undefined") {
      throw new Error(
        "PasskeyService can only be used in browser environments",
      );
    }
  ...
  private getPublicKey(response: AuthenticatorAttestationResponseJSON) {
    ...
    if (response.authenticatorData) {
      ...
      const attObj = cbor.decodeFirstSync(
  ...
  public async signPayload(payload: Buffer) {
    ...
    const compactSignature = this.compactSignature(
